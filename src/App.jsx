    ...
    <div className="app-container p-4 bg-anode-dark min-h-screen">
      ...
      <div className="features-card bg-anode-bg p-6 rounded-xl shadow-lg mb-6">
        ...
        <div className="feature-grid grid grid-cols-2 md:grid-cols-3 gap-4">
          <button onClick={navigateToMarketplace} className="btn bg-blue-600 hover:bg-blue-700 transition-colors py-3">🌐 Central Marketplace</button>
          <button onClick={navigateToEscrow} className="btn bg-green-600 hover:bg-green-700 transition-colors py-3">🔒 Escrow Services + Calculator</button>
          <button onClick={navigateToDAO} className="btn bg-purple-600 hover:bg-purple-700 transition-colors py-3">💡 Innovation Hub DAO</button>
          <button onClick={navigateToP2PTransfer} className="btn bg-yellow-600 hover:bg-yellow-700 transition-colors py-3">↔️ P2P $ANODE Transfer</button>
          <button onClick={navigateToStaking} className="btn bg-pink-600 hover:bg-pink-700 transition-colors py-3">🌱 $ANODE Staking</button>
          <button onClick={createDaoProposal} className="btn bg-orange-600 hover:bg-orange-700 transition-colors py-3">🗳️ DAO Proposal: African Tech Talents</button>
        </div>
      </div>
      ...
      {isAdmin && (
        <div className="admin-card bg-anode-bg p-6 rounded-xl shadow-lg border-2 border-anode-secondary">
          ...
          <p className="text-anode-text mb-4">Admin wallet **{connectedAddress.slice(0, 8)}...** detected.</p>
          <div className="admin-actions space-y-4">
            <button onClick={sendMint} className={`btn bg-anode-mint hover:bg-anode-mint-dark ${!connected ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!connected}>Mint 1000 $ANODE Tokens</button>
            <button onClick={sendAirdrop} className={`btn bg-anode-airdrop hover:bg-anode-airdrop-dark ${!connected ? 'opacity-50 cursor-not-allowed' : ''}`} disabled={!connected}>Airdrop 500 $ANODE Tokens</button>
          </div>
        </div>
      )}
    </div>
