...
export function useMainContract() {
  ...
  return {
    ...
    // Separate release for Team & Advisors (with cliff)
    executeReleaseTeam: async () => {
      ...
      return sender.send({
        ...
        value: toNano("0.1"),
        body: beginCell()
          .storeUint(0x52656c656173655f7465616d, 32) // "Release_team" opcode - adjust in tact
          .endCell()
      });
    },

    // Separate release for SAFT investors (zero cliff)
    executeReleaseSaft: async () => {
      ...
      return sender.send({
        ...
        value: toNano("0.1"),
        body: beginCell()
          .storeUint(0x52656c656173655f73616674, 32) // "Release_saft" opcode - adjust in tact
          .endCell()
      });
    },
    ...
    executeAddMarketItem: async (title: string, priceTon: string) => {
      if (MARKETPLACE_ADDR.includes("REPLACE")) return;
      return sender.send({
        to: Address.parse(MARKETPLACE_ADDR),
        value: toNano("0.05"),
        body: beginCell()
          .storeUint(0x01, 32) // replace with actual add_listing opcode from tact
          .storeStringTail(title)
          .storeCoins(toNano(priceTon))
          .endCell()
      });
    },

    executeUpdateMarketPrice: async (itemId: number, newPriceTon: string) => {
      if (MARKETPLACE_ADDR.includes("REPLACE")) return;
      return sender.send({
        to: Address.parse(MARKETPLACE_ADDR),
        value: toNano("0.05"),
        body: beginCell()
          .storeUint(0x02, 32) // replace with actual update_price opcode
          .storeUint(itemId, 64)
          .storeCoins(toNano(newPriceTon))
          .endCell()
      });
    },
    ...
    executeRemoveMarketItem: async (itemId: number) => {
      ...
      return sender.send({
        ...
        value: toNano("0.05"),
        body: beginCell()
          .storeUint(0x03, 32) // replace with actual remove_listing opcode
          .storeUint(itemId, 64)
          .endCell()
      });
    }
