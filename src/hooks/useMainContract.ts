...
export function useMainContract() {
  ...
  useEffect(() => {
    // 1. Instant Identity Check (Works even offline/without contracts)
    if (connected && sender.address) {
      try {
        ...
        if (sender.address.equals(adminAddr)) {
          ...
          setMemberRank({ score: 0, rank: "Guest" });
        }
    ...
    async function fetchData() {
      ...
      try {
        ...
        // Fetch DAO Rank (Only if DAO is actually deployed)
        if (DAO_ADDR.startsWith("EQ") || DAO_ADDR.startsWith("0Q")) {
           const isNotAdmin = !sender.address.equals(Address.parse(ADMIN_WALLET));
           if (isNotAdmin) {
