...
const DAO_ADDR = "0QDfCEYFiy0F5ntz4MIpM_8ciKAmTZ-36fJ54Ay4IlbAyo4u"; 
...
export function useMainContract() {
  ...
  const [memberRank, setMemberRank] = useState<{score: number, rank: string}>({score: 0, rank: "Guest"});
  ...
  useEffect(() => {
    async function fetchData() {
      ...
      try {
        ...
        const dao = Address.parse(DAO_ADDR);
        ...
        // 2. Get DAO Rank & Score
        const rankRes = await client.runMethod(dao, "get_member_rank", [{ type: "slice", cell: userAddrCell }]);
        setMemberRank({
            score: Number(rankRes.stack.readBigNumber()),
            rank: rankRes.stack.readString()
        });

        const res = await client.runMethod(master, "get_counter");
        setCounter(Number(res.stack.readBigNumber()));
      } catch (e) { console.log("Syncing Afro-Node Ecosystem..."); }
    }
    fetchData();
  ...
  return {
    ...
    executeAnodePayment: async (type: string, id: number, price: string) => {
      const destination = type === 'marketplace' ? MARKETPLACE_ADDR : "REPLACE_WITH_ESCROW";
